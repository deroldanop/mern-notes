...
export default function FileUploader({ attachments = [], onChange }) {
  ...
  const formatMicTime = (s) => `${String(Math.floor(s / 60)).padStart(2, "0")}:${String(s % 60).padStart(2, "0")}`
    ...
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {micRecording ? (
          <button
            type="button"
            className="btn btn-sm btn-error gap-1 animate-pulse"
            onClick={stopMicRecording}
          >
            {icons.stop} Stop {formatMicTime(micSeconds)}
          </button>
        ) : (
          <button
            type="button"
